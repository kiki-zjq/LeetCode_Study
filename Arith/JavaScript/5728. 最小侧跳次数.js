...
var minSideJumps = function(obstacles) {
    ...
    const MAX = 1e6;
    const dp = new Array(N).fill(0).map(v => new Array(4).fill(0));
    // 初始化
    dp[0][1] = dp[0][3] = 1;
    // 状态转移
    for (let i = 1; i < N; i++) {
      ...
      if (stone === 1) {
        ...
        dp[i][1] = Math.min(dp[i-1][1], dp[i-1][2]+1, dp[i-1][3]+1);
        dp[i][2] = Math.min(dp[i-1][2], dp[i-1][1]+1, dp[i-1][3]+1);
        dp[i][3] = Math.min(dp[i-1][3], dp[i-1][1]+1, dp[i-1][2]+1);
      }
    }
    // 返回结果
    return Math.min(...dp[N-1].slice(1));
  };
